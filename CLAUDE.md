...
- [x] **Stat-block paste/import** — ✅ done 2026-06-02. "📋 Paste Stat Block" accepts the skill's **structured JSON** export *or* its **Markdown** blocks (auto-detected), previews the parse, then saves to My Bestiary (+ optional add to encounter). Closes the loop between the two tools.
...
3. Bump `CACHE_NAME` in `sw.js` if deploying so installed PWAs pull the new shell (currently `tor2e-lm-v3`).

---

## Changelog

- **2026-06-02 — Stat-block paste/import (closes the NotebookLM loop).** New **📋 Paste Stat Block** button in the Add Adversaries card (`openStatBlockImportModal`). Accepts output from the `tor2e-adversary-statblock` skill in **either** format — the **structured JSON** export *or* the **Markdown** block(s) — auto-detected by `importAdversaries(text)` (JSON if it parses with an `adversaries` array; else the Markdown parser `parseAdversaryStatBlocks`). Both map onto the existing `ADVERSARY_DB` template shape. JSON mapping (`mapStructuredAdversaries`): `resourceType` → `hate`/`resolve`; `parry:null` → 0; `inheritedAbilities` resolved against each category's `sharedAbilities` (descriptions filled, tagged `shared:true`) and ordered before the foe's specific abilities; `uncertain` Moria weapons keep `rawGlyphs` + an `uncertain` flag. Flow: paste → **preview** (per-foe badges FOE/TEMPLATE/LORE, ⚠️ for estimate weapons, combat-ready vs reference counts) → choose **📚 Bestiary only** or **⚔️ Bestiary + Encounter**. `commitImportedAdversaries` saves every entry to My Bestiary (`_lib:true`, `_imported:true`; name collisions with built-ins get a " (Imported)" suffix so `addAdversaryToCombat` uses the imported stats, not the shadowed built-in) and optionally drops combat-ready standard foes into the encounter. **Non-combat entries** (`type:"template"` like Nameless Things, `type:"lore"` like the Beast of the Greyflood) save as **reference cards**: `populateAdvDb` shows a **📖 View** button (→ `viewBestiaryReference` modal: Table 6–8 notes / lore text / fixed abilities) instead of **+ Add**, and `addAdversaryToCombat` refuses them. Verified end-to-end in a jsdom harness against a 7-entry fixture spanning every shape (Resolve foe, multi-weapon Hate foe, ⚠️ Moria estimates, shared-ability inheritance, universal-only Undead, Nameless Things template, lore-only) + the Markdown path: 7 saved, 5 combat-ready added, refs non-addable, stats/flags correct. JS parses clean; synced to `index.html`; cache bumped `tor2e-lm-v2` → **`tor2e-lm-v3`**.
- **2026-06-01 — Fixed Loot tab crash when `history` missing (no hoard output).** If a saved `tor2e-lm-loot-v1` existed but lacked a `history` array (older/partial save, or a clobbered backup), `state.loot.history` was `undefined`: `loadAllState`→`renderLootTab`→`renderLootHistory` threw on `.length` during `window.onload`, and **`rollLootHoard()` threw on `.unshift` before it could show the result modal** — so "Roll Hoard Loot" produced *no screen / no output*. `loadAllState` now normalises loot after parsing (`state.loot` → object, `state.loot.history` → array), matching the existing convention used for bestiary/presets/encounter. Belt-and-braces guards also added to `renderLootHistory()` (the common render path) and `rollLootHoard()` so a later clobber can't re-break them. Reproduced + verified the fix in a jsdom harness across clean, `history` absent, and `history:null` states (onload clean, modal shows, generator export still works). JS parses clean; synced to `index.html`. Cache stays `tor2e-lm-v2`.
